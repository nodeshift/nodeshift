...
module.exports = async function getIngress (config, ingressResource) {
  ...
  let ingress = await awaitRequest(config.openshiftRestClient.apis.extensions.v1beta1.ns(config.namespace).ingresses(ingressResource.metadata.name).get());
  if (ingress.code === 404) {
    // There isn't a ingress yet, so we need to create one
    log.info(`creating new ingress ${ingressResource.metadata.name}`);
    ingress = await config.openshiftRestClient.apis.extensions.v1beta1.ns(config.namespace).ingresses.post({ body: ingressResource });
  } else {
