...
module.exports = async function getSecrets (config, secretResource) {
  ...
  const secret = await awaitRequest(config.openshiftRestClient.api.v1.ns(config.namespace).secrets(secretResource.metadata.name).get());
  if (secret.code === 404) {
    // There isn't a secret yet, so we need to create one
    log.info(`creating new secret ${secretResource.metadata.name}`);
    return config.openshiftRestClient.api.v1.ns(config.namespace).secrets.post({ body: secretResource });
  }
