...
async function deploy (config, deploymentConfigResource) {
  ...
  const deployment = await awaitRequest(config.openshiftRestClient.apis['apps.openshift.io'].v1.ns(config.context.namespace).deploymentconfigs(deploymentConfigResource.metadata.name).get());
  ...
  if (deployment.code === 404) {
    ...
    return config.openshiftRestClient.apis['apps.openshift.io'].v1.ns(config.context.namespace).deploymentconfigs.post({ body: deploymentConfigResource });
  }
...
async function undeploy (config, deploymentConfigResource) {
  ...
  response.deploymentConfig = await awaitRequest(config.openshiftRestClient.apis['apps.openshift.io'].v1.ns(config.context.namespace).deploymentconfigs(deploymentConfigResource.metadata.name).delete({ body: removeOptionsDeploymentConfig }));
  // Get the list of replication controllers
  const rcList = await config.openshiftRestClient.api.v1.ns(config.context.namespace).replicationcontrollers.get({ qs: { labelSelector: `openshift.io/deployment-config.name=${config.projectName}` } });
  ...
  for (const items of rcList.body.items) {
    ...
    response.replicationControllers.push(
      await awaitRequest(config.openshiftRestClient.api.v1.ns(config.context.namespace).replicationcontrollers(items.metadata.name).delete({ body: removeOptionsReplicationControllers }))
    );
  }
  return response;
}
