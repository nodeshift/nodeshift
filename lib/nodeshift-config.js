...
async function setup (options = {}) {
  ...
  dbg(`current cluster ${currentCluster}`);
  const config = {
    namespace: {
      name: currentCluster.namespace
    }
  };
  dbg(`config object created, namespace name same as current cluster's namespace ${format(config)}`);
  ...
  if (options.namespace) {
    ...
    dbg(`user provided option.namespace ${format(config.namespace)}`);
  } else {
  ...
  dbg(`options.outputImageStreamName ${options.outputImageStreamName}`);
  options.outputImageStreamTag = options.outputImageStreamTag || 'latest';
  dbg(`options.outputImageStreamTag ${options.outputImageStreamTag}`);
  ...
  dbg(`options.build ${format(options.build)}`);
  ...
  dbg(`a new object with the config, rest client and other data ${format(result)}`);
