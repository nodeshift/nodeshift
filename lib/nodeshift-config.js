...
async function setup (options = {}) {
  ...
  // TODO: this needs to be changed to use the kubernetes-client config stuff instead
  // kubernetes-client.config doesn't actually return any of the context info, so we still need to call the openshift-config-loader
  // Perhaps consider using the kubernetes-client inside the openshift-config-loader?
  ...
  const fullKubeConfig = k8sConfig.loadKubeconfig();
  const currentContext = fullKubeConfig['current-context'];
  const context = fullKubeConfig.contexts.find((c) => {
    return c.name === currentContext;
  });
  ...
  return Object.assign({}, config, {
    ...
    openshiftRestClient: await restClient({ config: k8sConfig.fromKubeconfig() })
  }, options);
}
