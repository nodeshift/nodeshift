...
async function setup (options = {}) {
  ...
  const config = k8sConfig.fromKubeconfig(options.config);
  ...
  if (options.namespace) {
    ...
    config.namespace = options.namespace.name;
  }

  logger.info(`using namespace ${config.namespace} at ${config.url}`);
  ...
  return Object.assign({}, config, {
    ...
    openshiftRestClient: await restClient({ config: config })
  }, options);
}
