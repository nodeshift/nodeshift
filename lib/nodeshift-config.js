...
function setup (options) {
  ...
  return openshiftConfigLoader(options.configLocation).then(config => {
    ...
    return openshiftRestClient(config).then(osc => {
      ...
      return Object.assign({}, config, {projectName: projectName, projectVersion: projectVersion, buildName: buildName, projectLocation: projectLocation, openshiftRestClient: osc});
    });
  });
}
