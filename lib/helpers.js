...
async function createDir (directoryToCreate) {
  try {
    await promisify(mkdirp)(directoryToCreate);
  } catch (err) {
    return Promise.reject(err);
  }
}

async function cleanUp (directoryToClean) {
  try {
    await promisify(rimraf)(directoryToClean);
  } catch (err) {
    return Promise.reject(err);
  }
}
