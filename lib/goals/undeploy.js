...
module.exports = async function (config) {
  ...
  for (const item of resourceList.items || []) {
    ...
    switch (item.kind) {
      case 'Route':
        response.route = await awaitRequest(config.openshiftRestClient.apis.route.v1.ns(config.context.namespace).routes(item.metadata.name).delete({ body: removeOptions }));
      ...
      case 'Service':
        ...
        response.service = await awaitRequest(config.openshiftRestClient.api.v1.ns(config.context.namespace).service(item.metadata.name).delete({ body: removeOptions }));
        break;
      ...
      case 'Secret':
        ...
        response.secret = await awaitRequest(config.openshiftRestClient.api.v1.ns(config.context.namespace).secrets(item.metadata.name).delete({ body: removeOptions }));
        break;
      case 'Ingress':
        // response.ingress = await config.openshiftRestClient.ingress.remove(item.metadata.name, removeOptions);
        response.ingress = await awaitRequest(config.openshiftRestClient.apis.extensions.v1beta1.ns(config.context.namespace).ingresses(item.metadata.name).delete({ body: removeOptions }));
        break;
      case 'ConfigMap':
        // response.configMap = await config.openshiftRestClient.configmaps.remove(item.metadata.name, removeOptions);
        response.configMap = await awaitRequest(config.openshiftRestClient.api.v1.ns(config.context.namespace).configmaps(item.metadata.name).delete({ body: removeOptions }));
        break;
      default:
