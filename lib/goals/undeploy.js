...
module.exports = async function (config) {
  ...
  for (const item of resourceList.items || []) {
    ...
    switch (item.kind) {
      case 'Route':
        response.route = await awaitRequest(config.openshiftRestClient.apis.route.v1.ns(config.namespace).routes(item.metadata.name).delete({ body: removeOptions }));
        break;
      case 'Service':
        response.service = await awaitRequest(config.openshiftRestClient.api.v1.ns(config.namespace).service(item.metadata.name).delete({ body: removeOptions }));
        break;
      case 'DeploymentConfig':
        response.deploymentConfig = await undeploy(config, item);
        break;
      case 'Secret':
        response.secret = await awaitRequest(config.openshiftRestClient.api.v1.ns(config.namespace).secrets(item.metadata.name).delete({ body: removeOptions }));
        break;
      case 'Ingress':
        response.ingress = await awaitRequest(config.openshiftRestClient.apis.extensions.v1beta1.ns(config.namespace).ingresses(item.metadata.name).delete({ body: removeOptions }));
        break;
      case 'ConfigMap':
        // response.configMap = await config.openshiftRestClient.configmaps.remove(item.metadata.name, removeOptions);
        response.configMap = await awaitRequest(config.openshiftRestClient.api.v1.ns(config.namespace).configmaps(item.metadata.name).delete({ body: removeOptions }));
        break;
      default:
