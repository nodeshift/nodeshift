...
const { wait, awaitRequest } = require('./helpers');
...
async function podLogEndpoint (config, build) {
  let response;
  try {
    response = await config.openshiftRestClient.api.v1.ns(config.context.namespace).pods(`${build}-build`).log.get();
  } catch (err) {
    response = err;
  }
...
module.exports = async function watchBuildLog (config, build) {
  ...
  const response = config.openshiftRestClient.api.v1.ns(config.context.namespace).pods(`${build}-build`).log.getStream({ qs: { follow: true, pretty: false } });
