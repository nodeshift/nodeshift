...
async function removeBuildsAndBuildConfig (config) {
  ...
  const buildList = await config.openshiftRestClient.apis.build.v1.ns(config.namespace).builds.get({ qs: { labelSelector: `openshift.io/build-config.name=${buildName}` } });
  ...
  for (const items of buildList.body.items) {
    ...
    await awaitRequest(config.openshiftRestClient.apis.build.v1.ns(config.namespace).builds(items.metadata.name).delete({ body: removeOptions }));
  }
  // delete build config
  return awaitRequest(config.openshiftRestClient.apis.build.v1.ns(config.namespace).buildconfigs(buildName).delete({ body: removeOptions }));
}
...
async function createOrUpdateBuildConfig (config) {
  ...
  const buildConfig = await awaitRequest(config.openshiftRestClient.apis.build.v1.ns(config.namespace).buildconfigs(buildName).get());
  ...
  if (buildConfig.statusCode === 404) {
    ...
    return config.openshiftRestClient.apis.build.v1.ns(config.namespace).buildconfigs.post({ body: newBuildConfig });
  }
