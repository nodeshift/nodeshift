...
async function removeBuildsAndBuildConfig (config) {
  ...
  const buildList = await config.openshiftRestClient.apis.build.v1.ns(config.context.namespace).builds.get({ qs: { labelSelector: `openshift.io/build-config.name=${buildName}` } });
  ...
  for (const items of buildList.body.items) {
    ...
    await awaitRequest(config.openshiftRestClient.apis.build.v1.ns(config.context.namespace).builds(items.metadata.name).delete({ body: removeOptions }));
  }
  // delete build config
  return awaitRequest(config.openshiftRestClient.apis.build.v1.ns(config.context.namespace).buildconfigs(buildName).delete({ body: removeOptions }));
...
async function createOrUpdateBuildConfig (config) {
  ...
  const buildConfig = await awaitRequest(config.openshiftRestClient.apis.build.v1.ns(config.context.namespace).buildconfigs(buildName).get());

  config.build = config.build || {};

  if (buildConfig.statusCode === 404) {
    ...
    return config.openshiftRestClient.apis.build.v1.ns(config.context.namespace).buildconfigs.post({ body: newBuildConfig });
  }
