...
module.exports = async function getConfigMap (config, configMapResource) {
  ...
  let configMap = await awaitRequest(config.openshiftRestClient.api.v1.ns(config.namespace).configmaps(configMapResource.metadata.name).get());
  if (configMap.code === 404) {
    ...
    configMap = await awaitRequest(config.openshiftRestClient.api.v1.ns(config.namespace).configmaps().post({ body: configMapResource }));
  } else {
