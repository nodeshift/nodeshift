...
// function dorequest (config, archiveLocation) {
//   return new Promise((resolve, reject) => {
//     const req = {
//       method: 'POST',
//       url: 'https://192.168.42.221:8443/apis/build.openshift.io/v1/namespaces/myproject/buildconfigs/nodejs-rest-http-s2i/instantiatebinary',
//       auth: {
//         bearer: config.user.token
//       },
//       json: false,
//       body: fs.createReadStream(archiveLocation),
//       strictSSL: false
//     };
...
module.exports = async (config, archiveLocation) => {
  ...
  const binaryResponse = await config.openshiftRestClient.apis.build.v1.ns(config.context.namespace).buildconfigs(config.buildName).instantiatebinary.post({ body: fs.createReadStream(archiveLocation), json: false /*, headers: { 'content-type': 'application/x-tar' } */ });
