...
module.exports = async (config, archiveLocation) => {
  ...
  const binaryResponse = await config.openshiftRestClient.apis.build.v1.ns(config.namespace).buildconfigs(config.buildName).instantiatebinary.post({ body: fs.createReadStream(archiveLocation), json: false });
  ...
  for (let i = 0; i <= MAX_RETRIES; i++) {
    ...
    const buildStatus = await config.openshiftRestClient.apis.build.v1.ns(config.namespace).builds(response.metadata.name).get();
