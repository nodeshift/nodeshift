...
module.exports = async function getServices (config, serviceResource) {
  const service = await awaitRequest(config.openshiftRestClient.api.v1.ns(config.namespace).service(serviceResource.metadata.name).get());
  if (service.code === 404) {
    // There isn't a service yet, so we need to create one
    log.info(`creating new service ${serviceResource.metadata.name}`);
    return config.openshiftRestClient.api.v1.ns(config.namespace).service.post({ body: serviceResource });
  }
