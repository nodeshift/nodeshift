...
async function createOrUpdateImageStream (config) {
  ...
  const imageStream = await awaitRequest(config.openshiftRestClient.apis.image.v1.ns(config.context.namespace).imagestreams(config.projectName).get());
  if (imageStream.code === 404) {
    // Need to create the image stream
    logger.info(`creating image stream ${config.projectName}`);
    return config.openshiftRestClient.apis.image.v1.ns(config.context.namespace).imagestreams.post({ body: createImageStream(config) });
  }
...
function removeImageStream (config) {
  ...
  // delete image stream
  return awaitRequest(config.openshiftRestClient.apis.image.v1.ns(config.context.namespace).imagestreams(config.projectName).delete({ body: removeOptions }));
