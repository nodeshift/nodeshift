...
const dockerfileWriter = require('./dockerfile-writer');
...
async function createArchive (config) {
  ...
  if (config.build.strategy && config.build.strategy.toUpperCase() === 'DOCKER') {
    // We need to be sure to 'await' for the tar.create before to update otherwise 
    // we include Dockerfile but end up missing some previous files.
    await tar.create({
    ...
    // Checks if user don't have a Dockerfile
    if (!fs.existsSync(`${config.projectLocation}/Dockerfile`)) {
      await dockerfileWriter.writeDockerfile(config);
    }
    
    return tar.update({
      cwd: `${config.projectLocation}/tmp`,
      file: `${config.projectLocation}/${DEFAULT_BUILD_LOCATION}/archive.tar`
    }, ['Dockerfile']);
  } else {
    return tar.create({
